...
delta=1e-4;
max_undo = 20;
curvature_scale = 100; %may need to change axes limits below
xmax = 1.2;
xmin = -0.5;
ymax =  .4;
ymin = -.2;
...
xs_top = xs(:,1:LEpos-1);
ys_top = ys(:,1:LEpos-1);
xs_bot = xs(:,LEpos+1:end);
ys_bot = ys(:,LEpos+1:end);
curvature_vectors_top = spline_curvature(xs_top,ys_top);
curvature_vectors_top = curvature_vectors_top/curvature_scale;
curvature_vectors_bot = spline_curvature(xs_bot,ys_bot);
curvature_vectors_bot = -curvature_vectors_bot/curvature_scale;
        ...
        case 'normal'
            ...
            if V2>min(1e-4,mindist2)
                ...
                if V2<1e-4
                    ...
                    nps_half = (length(xs)-1)/2;
                    xs_top = xs(:,1:nps_half+1);
                    ys_top = ys(:,1:nps_half+1);
                    curvature_vectors_top = spline_curvature(xs_top,ys_top);
                    curvature_vectors_top = curvature_vectors_top/curvature_scale;
